...
func FollowerList(ctx context.Context, c *app.RequestContext) {
	...
	if userID == "" {
		...
		ResponseError(c, http.StatusBadRequest,
			response.PackListError("user_id不能为空"))
		return
	}
	token := c.Query("token")
	if token == "" {
		...
		ResponseError(c, http.StatusBadRequest,
			response.PackListError("token不能为空"))
		return
	}
...
func RelationAction(ctx context.Context, c *app.RequestContext) {
	...
	if toUserID == "" {
		...
		ResponseError(c, http.StatusBadRequest,
			response.PackActionError("to_user_id不能为空"))
		return
	}
	tid, err := strconv.ParseInt(toUserID, 10, 64)
	if err != nil {
		logger.Errorf("failed to parse user_id: %v", err)
		ResponseError(c, http.StatusOK,
			response.PackActionError("to_user_id 不合法"))
		return
	}
	actionType, err := strconv.ParseInt(c.Query("action_type"), 10, 64)
	if err != nil || (actionType != 1 && actionType != 2) {
		ResponseError(c, http.StatusOK,
			response.PackActionError("action_type 不合法"))
		return
	}
	token := c.Query("token")
	if token == "" {
		logger.Info("Input token is empty")
		ResponseError(c, http.StatusBadRequest,
			response.PackPublishListError("token不能为空"))
		return
	}
